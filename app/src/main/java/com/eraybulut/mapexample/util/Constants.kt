package com.eraybulut.mapexample.util

/**
 * Created by Eray BULUT on 13.08.2023
 * eraybulutlar@gmail.com
 */

object Constants {
    const val BASE_MAP_URL = "https://maps.googleapis.com/maps/api/"
    const val BASE_IMAGE_URL = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference="
    const val CONNECT_TIMEOUT = 20L
    const val READ_TIMEOUT = 60L
    const val WRITE_TIMEOUT = 120L
    const val REQUEST_LOCATION_PERMISSION = 123
}